...
- 队列和 BFS
	...
	- BFS 在树中执行层序遍历
	- 使用 BFS 遍历图
	- 使用 BFS 找到从起始结点到目标结点的路径，特别是最短路径
	- 在更抽象的情景中使用 BFS 遍历所有可能的状态
	- 扩展：双端队列 | 优先队列
...
- 栈和 DFS
	- 用于在树/图中遍历/搜索的另一种重要算法
	- 可以用 DFS 进行 前序遍历，中序遍历和后序遍历
	- 特性：除非我们到达最深的结点，否则我们永远不会回溯
	- 使用递归实现 DFS。栈在递归中起着重要的作用
	- 当我们递归地实现 DFS 时，似乎不需要使用任何栈。但实际上，我们使用的是由系统提供的隐式栈，也称为调用栈（Call Stack）
	- 递归的缺点：如果递归的深度太高，你将遭受堆栈溢出。 在这种情况下，您可能会希望使用 BFS，或使用显式栈实现 DFS。
	- 使用 while 循环和栈来模拟递归期间的系统调用栈

- DFS 和 BFS 区别
	- BFS永远不会深入探索，除非它已经在当前层级访问了所有结点
